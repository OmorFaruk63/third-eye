...
export default function Recordings() {
  ...
  const filteredRecordings = recordings.filter((r) => {
    ...
    if (qualityFilter === 'DRIVE') return Boolean(r.driveFileId || r.driveViewLink);
    return true;
  });
  ...
  return (
    <Box sx={{ maxWidth: 1280, mx: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Card sx={{ p: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1 }}>
          <Button
            ...
            sx={{
              background: qualityFilter === 'ALL' ? 'rgba(0,229,255,0.18)' : 'transparent',
              borderColor: qualityFilter === 'ALL' ? 'rgba(0,229,255,0.4)' : 'rgba(255,255,255,0.1)',
          ...
          <Button
            ...
            sx={{
              background: qualityFilter === '720p' ? 'rgba(0,229,255,0.18)' : 'transparent',
              borderColor: qualityFilter === '720p' ? 'rgba(0,229,255,0.4)' : 'rgba(255,255,255,0.1)',
          ...
          <Button
            ...
            sx={{
              background: qualityFilter === '480p' ? 'rgba(0,229,255,0.18)' : 'transparent',
              borderColor: qualityFilter === '480p' ? 'rgba(0,229,255,0.4)' : 'rgba(255,255,255,0.1)',
          ...
          <Button
            ...
            sx={{
              background: qualityFilter === 'DRIVE' ? 'rgba(0,230,118,0.18)' : 'transparent',
              borderColor: qualityFilter === 'DRIVE' ? 'rgba(0,230,118,0.4)' : 'rgba(255,255,255,0.1)',
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap', ml: 'auto' }}>
          <Button
            size="small"
            variant="outlined"
            onClick={() => handleSelectAllToggle(paginatedRecordings.map((r) => r._id))}
            disabled={paginatedRecordings.length === 0}
            startIcon={
              <Box sx={{
                width: 14, height: 14, borderRadius: '4px',
                border: '1.5px solid', borderColor: allVisibleSelected ? '#00e5ff' : '#64748b',
                background: allVisibleSelected ? '#00e5ff' : 'transparent',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                {allVisibleSelected && <Check size={10} color="#000" strokeWidth={3} />}
              </Box>
            }
            sx={{
              borderColor: 'rgba(255,255,255,0.1)',
              color: selectedRecordings.length > 0 ? '#00e5ff' : 'text.secondary',
              fontSize: '0.75rem',
            }}
          >
            {selectedRecordings.length > 0 ? `${selectedRecordings.length} Selected` : 'Select Page'}
          </Button>
          ...
          {selectedRecordings.length > 0 && (
            <Button
              ...
              sx={{ color: '#94a3b8', fontSize: '0.75rem', minWidth: 0, textDecoration: 'underline' }}
            >
              Deselect
            </Button>
          )}
          ...
          {selectedRecordings.length > 0 && (
            ...
            >
              {isDeleting ? 'Deleting...' : `Delete (${selectedRecordings.length})`}
            </Button>
          )}
          ...
          <Button
            ...
            sx={{
              ...
              fontSize: '0.75rem',
              '&:hover': { background: 'rgba(255,23,68,0.15)', borderColor: '#ff1744' },
            }}
          >
            Purge All
          </Button>
              ...
              return (
                <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={rec._id}>
                  <Card sx={{
                    ...
                    '&:hover': {
                      borderColor: isSelected ? '#00e5ff' : 'rgba(0,229,255,0.4)',
                    ...
                    >
                      ...
                      <Box
                        ...
                        sx={{
                          ...
                          border: isSelected ? '1px solid #00e5ff' : '1px solid rgba(255,255,255,0.25)',
                          background: isSelected ? '#00e5ff' : 'rgba(0,0,0,0.6)',
                      ...
                      >
                        <Check size={14} strokeWidth={3} style={{ opacity: isSelected ? 1 : 0 }} />
                      </Box>
                      ...
                      <Box sx={{
                        ...
                        fontSize: '0.625rem', fontFamily: '"JetBrains Mono", monospace', color: '#fff',
                      ...
                      <Box>
                        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 1, mb: 1 }}>
                          <Box sx={{ minWidth: 0, flex: 1 }}>
                            ...
                            <Typography variant="caption" sx={{ color: '#00e5ff', fontFamily: 'monospace', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.8, fontSize: '0.75rem', color: 'text.secondary' }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Smartphone size={13} color="#00e5ff" style={{ flexShrink: 0 }} />
                            <Typography variant="caption" sx={{ color: 'text.secondary', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              Unit ID: <span style={{ fontFamily: 'monospace', color: '#94a3b8' }}>{rec.deviceId}</span>
                            </Typography>
                          </Box>

                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Calendar size={13} color="#00e676" style={{ flexShrink: 0 }} />
                            <Typography variant="caption" sx={{ color: '#cbd5e1', fontFamily: 'monospace' }}>
                              {new Date(rec.uploadedAt).toLocaleString(undefined, {
                          ...
                          <Box sx={{ pt: 0.5 }}>
                            {rec.driveFileId ? (
                              <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.8, color: '#00e676', fontSize: '0.75rem', fontWeight: 600 }}>
                                <CheckCircle2 size={13} />
                                <span>Synced to Google Drive</span>
                              </Box>
                            ) : (
                              <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.8, color: '#f59e0b', fontSize: '0.75rem' }}>
                                <Info size={13} />
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          {rec.driveViewLink ? (
                            <Button
                              ...
                              sx={{
                                fontSize: '0.75rem', py: 0.4, px: 1.2,
